...
import { useMealGuide } from '../hooks/useMeals';
...
export default function MealGuidePage() {
  ...
  const { data, isLoading } = useMealGuide(mealId);
  const [pickedCard, setPickedCard] = useState(null);

  const cautions = data?.cautions || [];
  const order = data?.eating_order || [];
            ...
            {order.length > 0 && (
              <div className="panel panel-cream">
                ...
                <ul style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {order.map((o) => (
                    <li key={o.step} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span
                        style={{
                          width: 24,
                          height: 24,
                      ...
                      <span style={{ fontSize: 14, fontWeight: 500 }}>{o.food_name}</span>
                      <span className="pill pill-outline" style={{ marginLeft: 'auto' }}>{o.category}</span>
                    </li>
                  ))}
                </ul>
              </div>
